...
const byte font_minus[3] = { B0001000, B0001000, B0001000 }; // Sinal de menos
const byte font[10][3] = {
  { B0111110, B0100001, B0111110 }, // 0
...
void prepareDisplayBuffer(int number) {
  ...
  if (number >= 10) {
    ...
    int ones = abs(number);
    // Exibe o sinal de menos (colunas 0, 1, 2)
    for (int i = 0; i < 3; i++) displayBuffer[i] = font_minus[i];
    // Exibe o dígito (colunas 4, 5, 6)
    for (int i = 0; i < 3; i++) displayBuffer[i + 4] = font[ones][i];
  }
}
...
void refreshDisplay() {
  ...
  if (millis() - lastDisplayRefresh > 2) {
    ...
    // Ativa apenas a coluna atual, invertendo a ordem para corrigir o espelhamento
